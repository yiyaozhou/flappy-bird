...
class Bird:
	IMGS = BIRD_IMGS
	# how much the bird gonna tilt
	MAX_ROTATION = 25
	ROT_VEL = 20
	ANIMATION_TIME = 5

	def __init__(self, x, y):
		# starting position of the birds
		self.x = x
		self.y = y
		self.tilt = 0
		self.tick_count = 0
	...
	def jump(self):
		# in pygame, move up, negative; move right, positive
		self.vel = -10.5
		self.tick_count = 0
		self.height = self.y
	...
	def move(self):
		...
		# displacement, how many pixels moving up or down
		d = self.vel * self.tick_count + 1.5 * self.tick_count ** 2
		# ex. -10.5 * 1 + 1.5 = -9

		# down greater than 16 pixels
		if d >= 16:
			d = 16
...
class Pipe:
	...
	def collide(self, bird):
		...
		b_point = bird_mask.overlap(bottom_mask, bottom_offset) # return None
